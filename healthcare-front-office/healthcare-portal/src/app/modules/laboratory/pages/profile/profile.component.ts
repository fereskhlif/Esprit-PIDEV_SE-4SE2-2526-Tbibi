...
@Component({
  ...
  template: `
    <div class="profile-wrapper">
      ...
      <ng-container *ngIf="!isLoading && profile">
        <div class="cover-banner"><div class="cover-orb cover-orb-1"></div><div class="cover-orb cover-orb-2"></div></div>
        <div class="profile-container" style="margin-top: -5rem;">
          <div class="profile-grid">
            <div class="card card-center">
              <div class="avatar-area">
                <div class="avatar-ring">
                  ...
                  <ng-template #defaultAvatar><span class="avatar-emoji">🔬</span></ng-template>
                  <div class="online-dot"></div>
                </div>
              </div>
              <h2 class="profile-name">{{ profile?.name }}</h2>
              <div class="role-badge"><span>Laboratory Staff</span></div>
            ...
            <div class="card card-right">
              <div class="section-block">
                ...
                <div class="form-group"><label>Full Name</label>
                  <div class="input-row"><input type="text" [(ngModel)]="editName" placeholder="Enter your name" />
                    <button class="save-btn" (click)="saveName()" [disabled]="savingName || !editName.trim()">{{ savingName ? 'Saving...' : 'Save' }}</button></div></div>
                <div class="form-group"><label>Email</label>
                  <div class="input-row"><input type="email" [(ngModel)]="editEmail" placeholder="Enter your email" />
                    <button class="save-btn" (click)="saveEmail()" [disabled]="savingEmail || !editEmail.trim()">{{ savingEmail ? 'Saving...' : 'Save' }}</button></div></div>
              ...
              <div class="section-block" style="margin-top: 2rem;">
                ...
                <button class="change-pw-btn" (click)="changePassword()" [disabled]="savingPw || !oldPassword || !newPassword || !confirmPassword">{{ savingPw ? 'Changing...' : 'Update Password' }}</button>
  ...
  `,
  styles: [`
    :host { display: block; font-family: system-ui, -apple-system, 'Inter', sans-serif; color: #0f172a; }
    ...
    .role-badge { display: inline-flex; padding: 0.3rem 0.75rem; background: #eff6ff; border-radius: 9999px; margin: 0.5rem auto 1rem; color: #2563eb; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
